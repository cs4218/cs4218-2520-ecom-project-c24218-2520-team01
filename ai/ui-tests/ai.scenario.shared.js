...
export const FORBIDDEN_UNRESOLVED_STEP_TOKENS = [
	'"target":',
	'"instruction":',
	'"assertion":',
];
...
export function resolveTemplatedString(value, variables, contextLabel) {
	...
	return value.replace(placeholderPattern, (match, key) => {
		if (!(key in variables)) {
			throw new Error(
				`Missing variable "${key}" referenced by ${contextLabel}.`,
			);
		}
...
function normalizeRelativeUrl(url, baseUrl) {
	...
	if (/^https?:\/\//i.test(trimmedUrl)) {
		...
		if (absoluteUrl.origin !== base.origin) {
			throw new Error(
				`Scenario URLs must stay on ${base.origin}. Received ${trimmedUrl}.`,
			);
		}
...
function assertValidSemanticStep(rawStep, index) {
	...
	if (!ALLOWED_SEMANTIC_STEP_TYPES.has(rawStep.type)) {
		throw new Error(
			`Step ${index + 1} has an unsupported type "${rawStep.type}".`,
		);
	}
}
...
export function normalizeSemanticScenario(
	rawScenario,
	variables = {},
	...
	if (!Array.isArray(rawScenario.steps) || rawScenario.steps.length === 0) {
		throw new Error(
			'The planner returned no executable steps in the "steps" array.',
		);
	}
