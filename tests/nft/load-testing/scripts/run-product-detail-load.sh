...
prune_old_product_detail_artifacts() {
  shopt -s nullglob
  local files=(
    "${PROFILE_RESULTS_DIR}/product-detail-"*.jtl
    "${PROFILE_RESULTS_DIR}/product-detail-"*-response-time-over-time.csv
    "${PROFILE_RESULTS_DIR}/product-detail-"*-response-time-by-endpoint.csv
  )
  local dirs=("${PROFILE_RESULTS_DIR}/product-detail-report-"*)
  if ((${#files[@]} > 0)); then
    rm -f "${files[@]}"
  fi
  if ((${#dirs[@]} > 0)); then
...
run_single_profile() {
  ...
  local jtl_file="${PROFILE_RESULTS_DIR}/product-detail-${profile_name}-${timestamp}.jtl"
  local report_dir="${PROFILE_RESULTS_DIR}/product-detail-report-${profile_name}-${timestamp}"
