...
case "${LOAD_PROFILE}" in
  baseline) DEFAULT_MIN_THROUGHPUT_REQ_PER_SEC=3 ;;
  funnel) DEFAULT_MIN_THROUGHPUT_REQ_PER_SEC=1 ;;
  *) DEFAULT_MIN_THROUGHPUT_REQ_PER_SEC=3 ;;
esac
...
prune_old_order_artifacts() {
  shopt -s nullglob
  local files=(
    "${PROFILE_RESULTS_DIR}/order-checkout-"*.jtl
    "${PROFILE_RESULTS_DIR}/order-checkout-"*-response-time-over-time.csv
    "${PROFILE_RESULTS_DIR}/order-checkout-"*-response-time-by-endpoint.csv
  )
  local dirs=("${PROFILE_RESULTS_DIR}/order-checkout-report-"*)
  if ((${#files[@]} > 0)); then
    rm -f "${files[@]}"
  fi
  if ((${#dirs[@]} > 0)); then
...
start_cpu_sampler() {
  ...
  mkdir -p "${RESULTS_DIR}"
  CPU_STOP_FILE="$(mktemp "${PROFILE_RESULTS_DIR}/cpu-stop-XXXXXX")"
  CPU_STATE_FILE="$(mktemp "${PROFILE_RESULTS_DIR}/cpu-state-XXXXXX")"
  rm -f "$CPU_STOP_FILE"
...
run_single_profile() {
  ...
  local jtl_file="${PROFILE_RESULTS_DIR}/order-checkout-${profile_name}-${timestamp}.jtl"
  local report_dir="${PROFILE_RESULTS_DIR}/order-checkout-report-${profile_name}-${timestamp}"
