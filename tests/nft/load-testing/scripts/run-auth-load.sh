...
case "${LOAD_PROFILE}" in
  ...
  funnel) DEFAULT_MIN_THROUGHPUT_REQ_PER_SEC=2 ;;
...
prune_old_auth_artifacts() {
  shopt -s nullglob
  local files=(
    "${PROFILE_RESULTS_DIR}/auth-flow-"*.jtl
    "${PROFILE_RESULTS_DIR}/auth-flow-"*-response-time-over-time.csv
    "${PROFILE_RESULTS_DIR}/auth-flow-"*-response-time-by-endpoint.csv
  )
  local dirs=("${PROFILE_RESULTS_DIR}/auth-flow-report-"*)
  if ((${#files[@]} > 0)); then
    rm -f "${files[@]}"
  fi
  if ((${#dirs[@]} > 0)); then
...
run_single_profile() {
  ...
  local jtl_file="${PROFILE_RESULTS_DIR}/auth-flow-${profile_name}-${timestamp}.jtl"
  local report_dir="${PROFILE_RESULTS_DIR}/auth-flow-report-${profile_name}-${timestamp}"
