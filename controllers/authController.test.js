...
describe("Unit test for updateProfileController", () => {
    ...
    test("Update profile successfully without password", async () => {
        // Arrange 
        const { password, ...inputWithoutPassword } = UPDATED_PROFILE_INPUT;
        req.body = inputWithoutPassword;
        ...
        expect(userModel.findByIdAndUpdate).toHaveBeenCalledWith(
            MOCK_USER._id,
            {
                name: inputWithoutPassword.name,
                password: MOCK_USER.password,
                phone: inputWithoutPassword.phone,
                address: inputWithoutPassword.address
            },
            { new: true }
        );
        expect(res.status).toHaveBeenCalledWith(200);
